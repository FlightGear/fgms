...
ProcessConfig ( const string& ConfigName )
{
  ...
  {
    // cout << "skipping " << ConfigName << " because config is already there!"
    //      << endl;
    return (0);
  }
  ...
  if (Val != "")
  {
    ...
    {
      cerr << "unknown value for 'server.daemon'!"
           << " in file " << ConfigName << endl;
    }
  }
  ...
  if (Val != "")
  {
    ...
    if (E)
    {
      cerr << "invalid value for tracking_port: '"
           << Val << "'" << endl;
      exit (1);
    }
...
//  string  Name = "";
  ...
  while (MoreToRead)
  {
    ...
    { 
      ...
      if (E)
      { 
        cerr << "invalid value for RelayPort: '" << Val << "'" << endl;
        exit (1);
      }
    }
    if ((Server != "") && (Port != 0))
    { 
      // cout << "Server: " << Server << " Port: " << Val << endl;
  ...
  while (MoreToRead)
  {
    ...
    if (Var == "crossfeed.port")
    {
      ...
      if (E)
      {
        cerr << "invalid value for crossfeed.port: '" << Val << "'" << endl;
        exit (1);
      }
    }
    if ((Server != "") && (Port != 0))
...
void SigHUPHandler ( int SigType )
{
  ...
  if (Servant.Init () != 0)
  {
    cerr << "received HUP signal, but reinit failed!" << endl;
    exit (1);
  }
